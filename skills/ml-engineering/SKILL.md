...
**Announce at start:** "I'm using the ml-engineering skill for ML-specific patterns."
...
- G → Run `systematic-debugging` first, then return with evidence
...
| Skill | Integration |
|-------|-------------|
| `test-driven-development` | Write data tests before pipeline, model tests before training |
| `systematic-debugging` | Use for training failures, accuracy drops |
| `verification-before-completion` | Verify metrics before claiming model works |
| `learning-from-experience` | Store patterns from failed experiments |
